...
internal fun PsiElement.getTestMethod(): Pair<PsiClass?, PsiMethod?> {
    val psiMethod = (parent.toUElement() as? UMethod)?.javaPsi
    if (psiMethod != null) {
        val psiClass = containingUClass()?.javaPsi
        if (psiClass != null) {
...
internal fun PsiElement.getTestClass(): PsiClass? {
    val psiClass = (parent.toUElement() as? UClass)?.javaPsi
    if (psiClass != null) {
        if (psiClass.isTestClass()) return psiClass
    }
    return null
}

internal fun PsiClass.isTestClass(): Boolean {
    val framework = TestFrameworks.detectFramework(this)
    if (framework?.isTestClass(this) == true) {
        return true
    }
    return false
}

internal val PsiElement.isIdentifier
    get() = toUElement() is UIdentifier

internal fun PsiElement.isPaparazziTestClass(): Boolean {
    val uClass = getUClass() ?: containingUClass()
    if (uClass != null) {
...
internal fun PsiElement.getUClass(): UClass? {
    return this.toUElement() as? UClass
}
